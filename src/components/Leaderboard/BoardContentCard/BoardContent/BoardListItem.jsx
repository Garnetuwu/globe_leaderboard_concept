...
const itemCard = 'bg-card-background w-[100%] text-center py-2'

const BoardListItem = (props) => {
    const { currentPageNumber } = useContext(LeaderboardContext)
    const top3 = props.index <= 2 && currentPageNumber === 1

    return (
        <>
            <GridRow className={`${top3 ? 'text-md' : 'text-sm'}`}>
                <div className={`${top3 ? 'text-lg' : 'text-md'} bg-card-background w-[70%] text-center rounded-md py-2 justify-self-start`}>
                    {`#${props.rank}`}
                </div>
                <div className={`${itemCard} rounded-l-md `}>
                ...
                <div className={itemCard}>{props.rating}</div>
                <div className={`${itemCard} rounded-r-md flex justify-between gap-2`}>
                    <span />
